...
tower_volume = H * W * W      # m^3, rectangular prism
...
def simulate_particle(diam, particleConcentration, particleDensity, y0, x0, particleCharge):
    ...
    t = 0.0
    xs, ys = [], []
    while t < air_cycle_time and x > 0 and y > 0:
        Fdx = -6 * math.pi * dynamicViscosity * R * vx
        Fdy = -6 * math.pi * dynamicViscosity * R * vy
        Fg = -m * g
        Fb = airDensity * V * g
        E_plate = sigma / (2 * epsilon_0)
        F_plate = particleCharge * E_plate
        E_cloud = (particleCharge * number_conc / (2 * epsilon_0)) * (2*x - H)
...
def get_removal_percentage(diam, particleConcentration, particleDensity, particleCharge, n_samples=50):
    ...
    for x0 in x_starts:
        ...
        if hit_plate:
            captured += 1
    removal_pct = (captured / n_samples) * 100
    return removal_pct, all_trajectories
...
def number_removed_per_cycle(diam, particleConcentration, particleDensity, particleCharge):
    ...
    mass_conc = particleConcentration * 1e-9      # kg/m^3
    number_conc = mass_conc / m                   # particles/m^3
    total_particles = number_conc * tower_volume  # particles in tower
...
pollutants = [
    {"name": "sulfur_dioxide",      "diam (nm)": 0.36,  "ug": 4.5,   "density": 544,  "charge_e": 200},
    {"name": "nitrogen_oxide",      "diam (nm)": 0.32,  "ug": 65,    "density": 363,  "charge_e": 200},
    {"name": "nitrogen_dioxide",    "diam (nm)": 0.33,  "ug": 63.2,  "density": 507,  "charge_e": 200},
    {"name": "carbon_monoxide",     "diam (nm)": 0.38,  "ug": 557.5, "density": 202,  "charge_e": 200},
    {"name": "ozone_particles",     "diam (nm)": 0.30,  "ug": 7.1,   "density": 704,  "charge_e": 200},
    {"name": "suspended_particles", "diam (nm)": 10000, "ug": 17.1,  "density": 1500, "charge_e": 5000},
    {"name": "fine_particles",      "diam (nm)": 2500,  "ug": 23.7,  "density": 1500, "charge_e": 800},
]
